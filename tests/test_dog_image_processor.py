...
class TestDogImageProcessorFileIO(unittest.TestCase):
    ...
    def test_save_image_creates_valid_jpeg_file(self):
        ...
        dog_img = ColorDogImage(img_array, "ссылка на кота", "кот")
    ...
    def test_saved_images_are_valid_image_files(self):
        ...
        """
        img = np.random.randint(0, 256, (24, 24, 3), dtype=np.uint8)
        dog = ColorDogImage(img, "ссылка", "порода")
        processed = [(0, dog, dog, dog)]
...
class TestDogImageProcessorAPI(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('async_processing.async_processing.implementation.dog_image_processor.aiohttp.ClientSession.get')
    async def test_download_data_returns_correct_structure(self, mock_get):
        ...
        self.assertTrue(
            len(results) == 1 and
            results[0][0] == 0 and
            results[0][1].url == "http://example.com/dog123.jpg" and
    ...
    async def test_download_data_makes_correct_api_calls(self, mock_get):
        ...
        self.assertTrue(
            mock_get.called and
            mock_get.call_args[0][0] == "https://api.thedogapi.com/v1/images/search" and
